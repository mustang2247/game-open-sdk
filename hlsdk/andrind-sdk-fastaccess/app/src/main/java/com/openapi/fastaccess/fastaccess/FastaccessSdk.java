...
public class FastaccessSdk {
    ...
    private LoginCallback loginCallbackImpl = new LoginCallback() {
        ...
        public void onLoginSuccess(UserLoginResponse userLoginResponse) {
            Log.d(Constants.tag, "登陆成功:" + JSON.toJSONString(userLoginResponse));
            try {
...
//				jo.put("customParams", "onLoginSuccess");
        ...
        public void onLoginFailed(String resultValue) {
            Log.d(Constants.tag, "登陆失败:" + resultValue);
            JSONObject jo = new JSONObject();
    ...
    public void doExit(final String SDKListening, final String exitCallbackMethod) {
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                Log.d(Constants.tag, "doExit：SDKListening=" + SDKListening + ",exitCallbackMethod=" + exitCallbackMethod);
                // 调用SDK退出功能
    ...
    public void doGetServers(final String SDKListening, final String callbackMethod, final String version) {
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                Log.d(Constants.tag, "doGetServers：SDKListening=" + SDKListening + ",callbackMethod=" + callbackMethod);
    ...
    public void doSelectServer(final String SDKListening, final String callbackMethod, final String serverId) {
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                Log.d(Constants.tag, "doSelectServer：SDKListening=" + SDKListening + ",callbackMethod=" + callbackMethod);
    ...
    public void doSendBIData(final String SDKListening, final String callbackMethod, final String metric, final String jsonString) {
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                Log.d(Constants.tag, "doSendBIData：SDKListening=" + SDKListening + ",callbackMethod=" + callbackMethod);
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    Log.e(Constants.tag, "setUserExtData:json=" + json, e);
                    showToast("数据扩展JSON格式错误!");
                    return;
                }
                Log.d(Constants.tag, "setExtData");
    ...
    public void androidKillProcess() {
        Log.d(Constants.tag, "androidKillProcess");
